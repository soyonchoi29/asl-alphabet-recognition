...
if __name__ == '__main__':
    ...
    X_kaggle = pickle.load(open('X_tsne_kaggle.sav', 'rb'))
    y_kaggle = pickle.load(open('y_tsne_kaggle.sav', 'rb'))

    X_webcam = pickle.load(open('X_tsne_webcam.sav', 'rb'))
    y_webcam = pickle.load(open('y_tsne_webcam.sav', 'rb'))
    ...
    for l, m in zip(np.unique(y_kaggle), markers):
        plt.scatter(X_kaggle[y_kaggle == l, 0], X_kaggle[y_kaggle == l, 1], c=colors[0], marker=m, label=l)

    # plot webcam data on same plot
    for l, m in zip(np.unique(y_webcam), markers):
        plt.scatter(X_webcam[y_webcam == l, 0], X_webcam[y_webcam == l, 1], c=colors[1], marker=m, label=l)
